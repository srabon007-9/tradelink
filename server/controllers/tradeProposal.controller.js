...
const tradeProposalController = {
  ...
  dispute: async (req, res, next) => {
    try {
      const proposal = await tradeProposalService.disputeProposal(req.params.id, req.user.id, req.body.reason);
      return res.status(200).json({
        success: true,
        message: 'Dispute submitted — an admin will review it shortly.',
        data: proposal,
      });
    } catch (err) {
      next(err);
