                ...
                {disputes.map(d => (
                  <tr key={d._id} className="hover:bg-concrete-50/50 transition-colors">
                    <td className="px-5 py-4 font-semibold text-navy-900">
                      {d.listingTitle || d.listing?.title || 'Skill Session'}
                    </td>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-concrete-200 pb-3">
              <h3 className="text-lg font-bold text-navy-900">
                Dispute Resolution — {selectedDispute.listingTitle || selectedDispute.listing?.title}
              </h3>
            ...
            <div className="space-y-3 text-xs bg-concrete-50 p-3 rounded-lg border border-concrete-200">
              ...
              <div className="flex justify-between">
                <span className="text-steel-600">Reported by:</span>
                <span className="font-semibold text-navy-900">{selectedDispute.disputedBy?.name || 'Unknown'}</span>
              </div>
            </div>

            {selectedDispute.disputeReason && (
              <div>
                <p className="mb-1 text-xs font-semibold text-steel-700">What went wrong (from the reporter):</p>
                <p className="rounded-lg border border-red-100 bg-red-50 p-3 text-xs text-red-900">
                  {selectedDispute.disputeReason}
                </p>
              </div>
            )}
